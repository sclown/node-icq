...
DEFAULTS = {
	URL: {
		...
		FILE_GATEWAY: 'http://api.icq.net/files/files/init',
		RAPI_BASE: 'http://rapi.icq.net',
...
ICQ.prototype.imageUpload = function (file, isSnap, callback) {
	...
	self.signedRequest({
		httpMethod: 'POST',
		json: true,
		url: DEFAULTS.URL.FILE_BASE + DEFAULTS.URL.FILE_GATEWAY,
		payload: sessionPayload,
		requestCallback: function (err, res, body) {
				body = body.data;
...
events.on('im:histDlgState', function (ctx, data) {
	...
	for (var msgIndex = 0; msgIndex < data.messages.length; msgIndex++) {
		if( parseInt(data.messages[msgIndex].msgId) > ctx.historyState[data.uin].lastMsgId) {
			data.class = evnt.eventData.messages[msgIndex].class;
			if(evnt.eventData.messages[msgIndex].chat) {
				data.sender = evnt.eventData.messages[msgIndex].chat.sender;
			}
			data.text = data.messages[msgIndex].text;
			if(data.messages[msgIndex].outgoing === true) {
				events.emit('im:mymessage', ctx, data);
			}
			else {
				events.emit('im:message', ctx, data);
			}
			ctx.historyState[data.uin].lastMsgId = evnt.eventData.messages[msgIndex].msgId;
		}
	}
